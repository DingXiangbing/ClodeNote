...
$(function(){
	...
	$(document).on('click','#modalBasic_11 .btn.btn-primary.sure',function(){
		//获取选中的option组件上绑定的笔记本id
		var id=$("#moveSelect option:selected").data("notebook");
		//获取要移动笔记的绑定的数据
		var note=$("#second_side_right .checked").parent().data("note");
		if(!id){//表示用户没有选择笔记本
			return;
		}
		note.cn_notebook_id=id;//改变该笔记对应的笔记本的id
		moveNote(note);
	});
	...
	$(document).on('click','#modalBasic_3 .btn.btn-primary.sure', function(){
		//获取选中的option组件上绑定的笔记本id
		var id=$("#replaySelect option:selected").data("notebook");
		//获取要移动笔记的绑定的数据
	...
	$(document).on("keyup", "body", function(e) {
		if($('#search_note').is(':focus')&&(e.keyCode==108||e.keyCode==13)){
			...
			if(text){
				//设置页面上div的现实与隐藏
				$('#pc_part_4,#pc_part_2,#pc_part_3,#pc_part_7,#pc_part_8').hide();
				$('#pc_part_5,#pc_part_6').show();
				$('#first_side_right li a').removeClass('checked');
				$('#rollback_button,#like_button,#action_button').removeClass('clicked');
				$('#sixth_side_right ul').empty();
				//将当前页初始化并绑定到[更多]按钮上
				$("#more_note").data("currentPage",1);
				getShareNoteList(text,1);
			}
		}
    }),
	//----更多搜索笔记
	$(document).on("click", "#more_note", function() {
			...
			getShareNoteList(text,currentPage);
    }),
	//----点击搜索笔记
